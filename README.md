# The C# Player’s Guide — Progress Repo

**Current position:** Part 2 — (Classes)  
**Last update:** 2026-02-21  
Solutions are under: [Challenges](./Challenges)

---

## Progress Checklist

### Part 1 — (Basics)
- [x] InstallingVisualStudio → [Folder](./Challenges/Part1-TheBasics/InstallingVisualStudio)
- [x] HelloWorld → [Folder](./Challenges/Part1-TheBasics/HelloWorld)
  - [x] HelloWorld → [Folder](./Challenges/Part1-TheBasics/HelloWorld/HelloWorld)
  - [x] TheMakingsOfAProgrammer → [Folder](./Challenges/Part1-TheBasics/HelloWorld/TheMakingsOfAProgrammer)
  - [x] WhatComesNext → [Folder](./Challenges/Part1-TheBasics/HelloWorld/WhatComesNext)
- [x] Comments → [Folder](./Challenges/Part1-TheBasics/Comments)
  - [x] TheThingNamer3000 → [Folder](./Challenges/Part1-TheBasics/Comments/TheThingNamer3000)
- [x] TheCSharpTypeSystem → [Folder](./Challenges/Part1-TheBasics/TheCSharpTypeSystem)
  - [x] TheVariableShop → [Folder](./Challenges/Part1-TheBasics/TheCSharpTypeSystem/TheVariableShop)
  - [x] TheVariableShopReturns → [Folder](./Challenges/Part1-TheBasics/TheCSharpTypeSystem/TheVariableShopReturns)
- [x] Math → [Folder](./Challenges/Part1-TheBasics/Math)
  - [x] TheDominionOfKings → [Folder](./Challenges/Part1-TheBasics/Math/TheDominionOfKings)
  - [x] TheFourSistersAndTheDuckbear → [Folder](./Challenges/Part1-TheBasics/Math/TheFourSistersAndTheDuckbear)
  - [x] TheTriangleFarmer → [Folder](./Challenges/Part1-TheBasics/Math/TheTriangleFarmer)
- [x] Console2.0 → [Folder](./Challenges/Part1-TheBasics/Console2.0)
  - [x] TheDefenseOfConsolas → [Folder](./Challenges/Part1-TheBasics/Console2.0/TheDefenseOfConsolas)
- [x] DecisionMaking → [Folder](./Challenges/Part1-TheBasics/DecisionMaking)
  - [x] RepairingTheClocktower → [Folder](./Challenges/Part1-TheBasics/DecisionMaking/RepairingTheClocktower)
  - [x] Watchtower → [Folder](./Challenges/Part1-TheBasics/DecisionMaking/Watchtower)
- [x] Switches → [Folder](./Challenges/Part1-TheBasics/Switches)
  - [x] BuyingInventory → [Folder](./Challenges/Part1-TheBasics/Switches/BuyingInventory)
  - [x] DiscountedInventory → [Folder](./Challenges/Part1-TheBasics/Switches/DiscountedInventory)
- [x] Loops → [Folder](./Challenges/Part1-TheBasics/Loops)
  - [x] TheMagicCannon → [Folder](./Challenges/Part1-TheBasics/Loops/TheMagicCannon)
  - [x] ThePrototype → [Folder](./Challenges/Part1-TheBasics/Loops/ThePrototype)
- [x] Arrays → [Folder](./Challenges/Part1-TheBasics/Arrays)
  - [x] ReplicatorOfDTo → [Folder](./Challenges/Part1-TheBasics/Arrays/ReplicatorOfDTo)
  - [x] TheLawsOfFreach → [Folder](./Challenges/Part1-TheBasics/Arrays/TheLawsOfFreach)
- [x] Methods → [Folder](./Challenges/Part1-TheBasics/Methods)
  - [x] Countdown → [Folder](./Challenges/Part1-TheBasics/Methods/Countdown)
  - [x] TakingANumber → [Folder](./Challenges/Part1-TheBasics/Methods/TakingANumber)
- [x] MemoryManagement → [Folder](./Challenges/Part1-TheBasics/MemoryManagement)
  - [x] HuntingTheManticore → [Folder](./Challenges/Part1-TheBasics/MemoryManagement/HuntingTheManticore)

### Part 2 — (Object-Oriented Programming)
- [x] Enumerations → [Folder](./Challenges/Part2-OOP/Enumerations)
  - [x] SimulasTest → [Folder](./Challenges/Part2-OOP/Enumerations/SimulasTest)
  - [x] TheColor → [Folder](./Challenges/Part2-OOP/Enumerations/TheColor)
- [x] Tuples → [Folder](./Challenges/Part2-OOP/Tuples)
  - [x] TheCard → [Folder](./Challenges/Part2-OOP/Tuples/TheCard)
- [ ] Classes → [Folder](./Challenges/Part2-OOP/Classes)
  - [ ] VinFletchersArrows → [Folder](./Challenges/Part2-OOP/Classes/VinFletchersArrows)
  - [ ] VinFletchersArrowsExpanded → [Folder](./Challenges/Part2-OOP/Classes/VinFletchersArrowsExpanded)
- [ ] InformationHiding → [Folder](./Challenges/Part2-OOP/InformationHiding)
  - [ ] ThePoint → [Folder](./Challenges/Part2-OOP/InformationHiding/ThePoint)
  - [ ] ThePasswordValidator → [Folder](./Challenges/Part2-OOP/InformationHiding/ThePasswordValidator)
  - [ ] TheDoor → [Folder](./Challenges/Part2-OOP/InformationHiding/TheDoor)
- [ ] Properties → [Folder](./Challenges/Part2-OOP/Properties)
  - [ ] RockPaperScissors → [Folder](./Challenges/Part2-OOP/Properties/RockPaperScissors)
- [ ] Static → [Folder](./Challenges/Part2-OOP/Static)
  - [ ] TheRobotPilot → [Folder](./Challenges/Part2-OOP/Static/TheRobotPilot)
  - [ ] TheRobotPilotExpanded → [Folder](./Challenges/Part2-OOP/Static/TheRobotPilotExpanded)
- [ ] CatacombsOfTheClass → [Folder](./Challenges/Part2-OOP/CatacombsOfTheClass)
  - [ ] PackingInventory → [Folder](./Challenges/Part2-OOP/CatacombsOfTheClass/PackingInventory)
  - [ ] LabelingInventory → [Folder](./Challenges/Part2-OOP/CatacombsOfTheClass/LabelingInventory)
- [ ] Inheritance → [Folder](./Challenges/Part2-OOP/Inheritance)
  - [ ] TheOldRobot → [Folder](./Challenges/Part2-OOP/Inheritance/TheOldRobot)
  - [ ] RobotsAndTheirCommands → [Folder](./Challenges/Part2-OOP/Inheritance/RobotsAndTheirCommands)
- [ ] Polymorphism → [Folder](./Challenges/Part2-OOP/Polymorphism)
  - [ ] ColoredItems → [Folder](./Challenges/Part2-OOP/Polymorphism/ColoredItems)
- [ ] Interfaces → [Folder](./Challenges/Part2-OOP/Interfaces)
  - [ ] TheLockedDoor → [Folder](./Challenges/Part2-OOP/Interfaces/TheLockedDoor)
- [ ] Structs → [Folder](./Challenges/Part2-OOP/Structs)
  - [ ] RoomCoordinates → [Folder](./Challenges/Part2-OOP/Structs/RoomCoordinates)
- [ ] Records → [Folder](./Challenges/Part2-OOP/Records)
  - [ ] WarPreparations → [Folder](./Challenges/Part2-OOP/Records/WarPreparations)
- [ ] GenericTypes → [Folder](./Challenges/Part2-OOP/GenericTypes)
  - [ ] TheSieve → [Folder](./Challenges/Part2-OOP/GenericTypes/TheSieve)
- [ ] TheFountainOfObjects → [Folder](./Challenges/Part2-OOP/TheFountainOfObjects)
  - [ ] TheFountainOfObjects → [Folder](./Challenges/Part2-OOP/TheFountainOfObjects/TheFountainOfObjects)
- [ ] SomeUsefulTypes → [Folder](./Challenges/Part2-OOP/SomeUsefulTypes)
  - [ ] NavigatingOperandCity → [Folder](./Challenges/Part2-OOP/SomeUsefulTypes/NavigatingOperandCity)
  - [ ] IndexingOperandCity → [Folder](./Challenges/Part2-OOP/SomeUsefulTypes/IndexingOperandCity)
### Part 3 — (Advanced Topics)
- [ ] ManagingLargerPrograms → [Folder](./Challenges/Part3-AdvancedTopics/ManagingLargerPrograms)
- [ ] MethodsRevisited → [Folder](./Challenges/Part3-AdvancedTopics/MethodsRevisited)
- [ ] Exceptions → [Folder](./Challenges/Part3-AdvancedTopics/Exceptions)
- [ ] Delegates → [Folder](./Challenges/Part3-AdvancedTopics/Delegates)
- [ ] Events → [Folder](./Challenges/Part3-AdvancedTopics/Events)
- [ ] LambdaExpressions → [Folder](./Challenges/Part3-AdvancedTopics/LambdaExpressions)
- [ ] Files → [Folder](./Challenges/Part3-AdvancedTopics/Files)
- [ ] Patterns → [Folder](./Challenges/Part3-AdvancedTopics/Patterns)
- [ ] OperatorOverloading → [Folder](./Challenges/Part3-AdvancedTopics/OperatorOverloading)
- [ ] QueryExpressions → [Folder](./Challenges/Part3-AdvancedTopics/QueryExpressions)
- [ ] Threads → [Folder](./Challenges/Part3-AdvancedTopics/Threads)
- [ ] AsynchronousProgramming → [Folder](./Challenges/Part3-AdvancedTopics/AsynchronousProgramming)
- [ ] DynamicObjects → [Folder](./Challenges/Part3-AdvancedTopics/DynamicObjects)
- [ ] BeyondASingleProject → [Folder](./Challenges/Part3-AdvancedTopics/BeyondASingleProject)
- [ ] Publishing → [Folder](./Challenges/Part3-AdvancedTopics/Publishing)

### Part 4 — (The Endgame)
- [ ] TheFinalBattle → [Folder](./Challenges/Part4-TheEndgame/TheFinalBattle)

---

## Notes
- (add quick bullets here)
